...
void main() {
  group('PointsSummarySection', () {
    ...
    testWidgets('a max-level member sees "Top level reached", not a broken next-level line', (
      tester,
    ) async {
      const summary = PointsSummary(
        totalPoints: 20000,
  ...
  group('PointsHistoryScreen', () {
    testWidgets('empty ledger shows the honest empty state, not a spinner or crash', (
      tester,
    ) async {
      await tester.pumpWidget(
        _plainHost(
    ...
    testWidgets('renders real entries grouped by day with human reason labels', (
      tester,
    ) async {
