...
void main() {
  group('commentMatchesBlocklist', () {
    ...
    test('matches a blocked term used as a whole word', () {
      expect(
        commentMatchesBlocklist('this trek was total shit', terms),
        isTrue,
      );
    });
    ...
    test('does not match clean text', () {
      expect(
        commentMatchesBlocklist('This trek was a great classic route!', terms),
        isFalse,
      );
    });

    test(
      'does not false-positive on an innocent word containing a term as a substring',
      () {
        ...
        expect(
          commentMatchesBlocklist('assassin assessment classic', ['ass']),
          isFalse,
        );
      },
    );
    ...
    test('does not match a blocked term embedded inside a longer word', () {
      expect(
        commentMatchesBlocklist('assholenot a real word', ['asshole']),
        isFalse,
      );
    });
    ...
    test('matches when the blocked term is its own word even mid-sentence', () {
      expect(
        commentMatchesBlocklist('you are an asshole honestly', ['asshole']),
        isTrue,
      );
    });
